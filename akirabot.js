...
//apikey
const lol = '4b449ff342fcda7a87de3b67'
...
//WWEB
akira.version = [2,2140,12] //Fix Bug
   akira.on('qr', qr => {
            ...
            switch(command) {
                ...
                case 'alquranaudio':
                    ...
                    try {
                        ...
                        akira.sendMessage(from,`*「 WAIT 」 DON'T SPAM*`, text)
                ...
                case 'ceknsfw':
                    ...
                    if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
                        ...
                        if (get_result.status == '200') {
                            ...
                            reply('Fiturnya lagi error ngab')
                        }
                    } else {
                        reply(`Kirim gambar dengan caption *${prefix + command}* atau tag gambar yang sudah dikirim`)
                    }
                ...
                case 'meme':
                    ...
                    if (args.length != 0) return reply(ind.noneed())
                    try {
                        ini_buffer = await getBuffer(`https://api.lolhuman.xyz/api/meme/memeindo?apikey=${lol}`)
                        await akira.sendMessage(from, ini_buffer, image, { quoted: mek })
                        await limitAdd(sender)   
                    } catch {
                        reply(ind.err())
                ...
                case 'meme2':
                    ...
                    if (args.length != 0) return reply(ind.noneed())
                    try {
                        ini_buffer = await getBuffer(`https://api.lolhuman.xyz/api/random/meme?apikey=${lol}`)
                        await akira.sendMessage(from, ini_buffer, image, { quoted: mek })
                        await limitAdd(sender)
                    } catch {
                        reply(ind.err())
                ...
                case 'darkjoke':
                    ...
                    if (args.length != 0) return reply(ind.noneed())
                    try {
                        ini_buffer = await getBuffer(`https://api.lolhuman.xyz/api/meme/darkjoke?apikey=${lol}`)
                        await akira.sendMessage(from, ini_buffer, image, { quoted: mek })
                        await limitAdd(sender)
                    } catch {
                        reply(ind.err())
...
` 
				...
				case 'bc': 
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						buff = await akira.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							akira.sendMessage(_.jid, buff, image, {caption: `*_Changelog v4 :_*\n\n${body.slice(4)}`})
						}
						reply('*_succes broadcast_* ')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `*_Changelog v4 :_*\n\n${body.slice(4)}`)
						}
						reply('*_succes broadcast_* ')
					}
					break
                ...
                default:
                    ...
                    if (!isGroup && !isCmd && !isMedia && !isQuotedImage && !isQuotedVideo && !isQuotedSticker && !isMediaStik) {
                        ...
                        simi = await fetchJson(`https://simsumi.herokuapp.com/api?text=${budy}&lang=id`)
                        reply(simi.success)
                    }
                }
